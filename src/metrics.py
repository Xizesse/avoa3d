...
class BasicAVOAMetrics(Node):
    def __init__(self):
        ...
        print(f"DEBUG: Got scenario={self.scenario}, results_dir={self.results_dir}")
        
        # Get topic names
        print("DEBUG: Getting topic names...")
        ...
        print(f"DEBUG: Got topics: {self.odometry_topic}, {self.cmd_vel_topic}")
        
        # Create timestamped results directory
        print("DEBUG: Creating results directory...")
        ...
        print(f"DEBUG: Created directory: {self.scenario_dir}")
        ...
        self.goal_odometry_sub = self.create_subscription(
            Odometry,
            self.goal_odometry_topic,
            self.goal_odometry_callback,
            10
        )
        ...
        self.get_logger().info(f"  - Odometry: {self.odometry_topic}")
    ...
    def odometry_callback(self, msg):
        ...
        # self.get_logger().debug(f"Received odometry: pos=({msg.pose.pose.position.x:.2f}, {msg.pose.pose.position.y:.2f})")

    def goal_odometry_callback(self, msg):
        """Callback for goal odometry"""
        self.current_goal_odometry = msg
        self.received_goal = True
        # self.get_logger().debug(f"Received goal: pos=({msg.pose.pose.position.x:.2f}, {msg.pose.pose.position.y:.2f})")

    def cmd_vel_callback(self, msg):
        """Callback for commanded velocity"""
        self.current_cmd_vel = msg
        self.received_cmd_vel = True
        # self.get_logger().debug(f"Received cmd_vel: vel=({msg.linear.x:.2f}, {msg.linear.y:.2f})")

    def desired_vel_callback(self, msg):
        """Callback for desired velocity"""
        self.current_desired_vel = msg
        self.received_desired_vel = True
        # self.get_logger().debug(f"Received desired_vel: vel=({msg.linear.x:.2f}, {msg.linear.y:.2f})")
    ...
    def generate_plots(self):
        ...
        try:
            # Convert data points to numpy arrays for easier processing
            data = np.array(self.data_points)
            
            # Extract data columns (matching CSV headers)
            time_elapsed = data[:, 1]
            pos_x = data[:, 2]
            pos_y = data[:, 3]
            goal_x = data[:, 5] if len(data[0]) > 5 else [10.0] * len(data)
            goal_y = data[:, 6] if len(data[0]) > 6 else [0.0] * len(data)
            distance_to_goal = data[:, -1]  # Last column
            
            # Create X,Y trajectory plot
            plt.figure(figsize=(10, 10))
            
            # Plot trajectory path
            plt.plot(pos_x, pos_y, 'b-', linewidth=2, alpha=0.8)
            
            # Mark start point (blue square, higher alpha)
            plt.scatter(pos_x[0], pos_y[0], color='blue', s=120, marker='s', 
                       alpha=0.8, zorder=5, edgecolors='darkblue', linewidth=2)
            
            # Mark goal point (red X) - keeping this for reference
            goal_x_val = goal_x[0] if hasattr(goal_x, '__len__') else goal_x
            goal_y_val = goal_y[0] if hasattr(goal_y, '__len__') else goal_y
            plt.scatter(goal_x_val, goal_y_val, color='red', s=150, marker='x', 
                       zorder=5, linewidth=3)
            
            # Mark end point (blue square, light transparent)
            plt.scatter(pos_x[-1], pos_y[-1], color='blue', s=120, marker='s', 
                       alpha=0.3, zorder=5, edgecolors='darkblue', linewidth=2)
            
            # Set square boundaries from -10 to 10
            plt.xlim(-10, 10)
            plt.ylim(-10, 10)
            ...
            # Labels with larger font sizes (no title)
            plt.xlabel('X Position (m)', fontsize=20, fontweight='bold')
            plt.ylabel('Y Position (m)', fontsize=20, fontweight='bold')
            
            # Make axis numbers (tick labels) bigger
            plt.tick_params(axis='both', which='major', labelsize=18)
            
            # Equal aspect ratio to make it truly square
            plt.axis('equal')
            ...
            self.generate_stats_file(time_elapsed, pos_x, pos_y, distance_to_goal)
            
            self.get_logger().info(f"📊 Trajectory plot saved to: {plot_filename}")
            
        except Exception as e:
            self.get_logger().error(f"📊 Error generating plots: {str(e)}")

    def generate_stats_file(self, time_elapsed, pos_x, pos_y, distance_to_goal):
