...
public:
    AVOA() : Node("avoa3dnode", rclcpp::NodeOptions())
    {
        ...
        std::cout << "  - Desired Velocity (TwistStamped): " << desired_vel_topic << std::endl;
        ...
        // Publishers and subscribers - updated desired velocity subscriber to TwistStamped
...
private:
    ...
    {
        // Extract the Twist part from TwistStamped and store it
        latest_desired_velocity_ = msg->twist;
        
        // Store the full TwistStamped for potential future use (timestamp, frame_id)
        latest_desired_velocity_stamped_ = *msg;
        
        // Pass the Twist part to the sample evaluator (maintaining existing functionality)
        sample_evaluator_->setDesiredVelocity(msg->twist);
        
        RCLCPP_DEBUG(this->get_logger(), "Updated agent desired velocity from TwistStamped (frame: %s, time: %d.%d)", 
    ...
    void timer_callback()
    {
        ...
        if (!has_received_all_data()) {
            RCLCPP_INFO(this->get_logger(), "MISSING DATA");
            return;
        }
    ...
    bool has_received_all_data()
    {
        ...
        static bool have_desired_velocity = true;
    ...
    geometry_msgs::msg::Twist latest_desired_velocity_{};  // Extracted Twist part (maintains compatibility)
    geometry_msgs::msg::TwistStamped latest_desired_velocity_stamped_{};  // Full TwistStamped for potential future use
